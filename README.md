...
A couple of FLA cutscenes (FLUTE2, GLASS2) are scored with a MIDI flute. flade
bundles a tiny soundfont for it (one synthesised GM-flute instrument, ~9 KB
baked into the binary), so it plays out of the box from `MIDI_MI.HQR` (read
automatically from `--cd`, or pass `--midi`). If you have a real General MIDI
soundfont installed it's used instead, or pass `--soundfont my.sf2` for a richer
flute. Steam/GOG re-releases ship MP3 music instead of the MIDI, so the flute is
only available from the CD data.
...
| `midi.c` | render FLA cutscene MIDI (XMI → SMF → TinySoundFont); bundled flute soundfont (`flute_sf2.h`, generated by `tools/make_flute_sf2.py`) |
